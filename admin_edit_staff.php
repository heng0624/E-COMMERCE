...
<?php   
    ...
    if (is_get()) {
        ...
        if (!$usersData) {
            echo "<script>alert('User not found');window.location.href='admin_staff.php';</script>";
            exit();
        }
     ...
     if (is_post() == 'POST') {
        ...
        if ($image['error'] == 4) { // No file uploaded
            if ($usersData['profile_image']== null) { // If no image exists in the database
                ...
                $imagePath = $usersData['profile_image'];
            }
        ...
        if (!$_err) {
            ...
            redirect("admin_staff.php?msg=updated");
 ...
 ?>
    ...
    <div class="container">
        ...
        <h1>View & Edit Staff</h1>

        <form method="post" action="admin_edit_staff.php" enctype="multipart/form-data">
            <div class="form-row">
                <div>
                ...
                <div>
                    <label>Staff ID &nbsp&nbsp<?= err('id') ?></label>
                    <input type="text" name="id"  value="<?= $usersData["userId"]?>"readonly>
                </div>
                <div>
                    <label>Privilege </label>
                    <select name="privilege">
                        <option value="Manager"<?= ($usersData['role'] === 'Manager') ? 'selected' : '' ?>>Manager</option>
                        <option value="Staff"<?= ($usersData['role'] === 'Staff') ? 'selected' : '' ?>>Staff</option>
                    </select>
                </div>
            </div>
            ...
            <input type="date" name="date" id="date" value="<?= $usersData["birthday"]?>"/>
