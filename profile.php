...
<?php include "header.php" ?>
    ...
    if (is_post()) {
        ...
        if ($image['error'] == 4) { // No file uploaded
            if (empty($customerData['profile_image'])) { // If no image exists in the database
    ...
    <div class="container">
        <aside class="sidebar">
            <div class="profile">
                <img src="<?= $customerData["profile_image"] ?>" alt="User Image" class="profile-img">
                <p class="username"><?= $customerData['userName'] ?></p>
            </div>
            <nav>
                <ul>
                    <li><a href="profile.php">Profile</a></li>
                    <!-- <li><a href="bank&Ewallet.php">Banks & e-Wallet</a></li> -->
        ...
        <main class="profile-container">
            <h2>PROFILE</h2>
            <form class="profile-form" id="profileForm" action="profile.php" method="POST" enctype="multipart/form-data">
                <div class="profile-image-section">
                    <img id="uploadedImage" src="<?= $customerData["profile_image"] ?>" alt="User Avatar" class="profile-img-large">
                ...
                <label>Name</label>
                <input type="text" placeholder="Enter your name" name="name" value="<?= $customerData['userName'] ?>"> <?= err('name') ?>

                <label>Email</label>
                <input type="email" placeholder="Enter your email" name="email" value="<?= $customerData['email'] ?>"> <?= err('email') ?>

                <label>Phone</label>
                <input type="tel" placeholder="Enter your phone number" name="phone" value="<?= $customerData['phone'] ?>"> <?= err('phone') ?>

                <label>Gender</label>
                <div class="gender-options">
                    <input type="radio" name="gender" id="male" value="M" <?= (isset($customerData['gender']) &&  $customerData['gender'] == 'M') ? 'checked' : '' ?>>
                    <label for="male">Male</label>
                    <input type="radio" name="gender" id="female" value="F" <?= (isset($customerData['gender']) &&  $customerData['gender'] == 'F') ? 'checked' : '' ?>>
                ...
                <div class="dob">
                    <input type="date" name="date" id="date" value="<?= $customerData['birthday'] ?>" /> <?= err('date') ?>
                </div>
